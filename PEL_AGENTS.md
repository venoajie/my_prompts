...
-   **Expected Inputs:**
    -   `code_to_audit` (primary): The source code file or directory to be audited for security vulnerabilities.
---
...
-   **Expected Inputs:**
    -   `performance_metrics` (primary): Performance artifacts like `EXPLAIN ANALYZE` output, profiler data, or load test results.
    -   `source_code` (optional): The specific source code file related to the identified bottleneck.
---
...
-   **Expected Inputs:**
    -   `documentation_goal` (primary): A mandate describing the document to be created and its target audience.
    -   `source_artifacts` (primary): A collection of technical artifacts (blueprints, code) to be used as the source of truth.
---
...
-   **Expected Inputs:**
    -   `architectural_blueprint` (primary): The document describing the system architecture to be analyzed for test planning.
    -   `directory_structure` (primary): The `tree` output of the codebase to understand module relationships.
---
...
-   **Expected Inputs:**
    -   `decision_context` (primary): A document or mandate describing the technical decision that needs to be made.
    -   `related_artifacts` (optional): Supporting evidence such as blueprints, roadmaps, or relevant source code.
---
...
-   **Expected Inputs:**
    -   `code_for_review` (primary): The source code file to be peer-reviewed.
---
...
-   **Expected Inputs:**
    -   `test_plan` (primary): A structured document outlining the test cases to be executed.
    -   `knowledge_base` (optional): A collection of artifacts (blueprints, code) needed to execute the test plan.
---
...
-   **Expected Inputs:**
    -   `failure_report` (primary): Logs, error messages, or a description of the critical system failure.
    -   `architectural_blueprint` (primary): The system blueprint used to form a hypothesis about the failure.
---
...
-   **Expected Inputs:**
    -   `architectural_blueprint` (primary): The document describing the system's components and dependencies.
---
...
-   **Expected Inputs:**
    -   `api_requirements` (primary): A document outlining the requirements for the new API endpoint or service.
---
...
-   **Expected Inputs:**
    -   `file_path` (primary): The relative path of the file to be analyzed (e.g., 'src/shared/models.py').
    -   `file_content` (primary): The full text content of the file to be analyzed.
---
...
-   **Expected Inputs:**
    -   `architectural_blueprint` (primary): The PEL_BLUEPRINT.md file, which is the ground truth for the audit.
    -   `directory_structure` (primary): The full output of a `tree` command for the repository.
    -   `core_tooling_and_docs` (primary): The source code for key scripts (e.g., pel_toolkit.py) and the root README.md.
---
